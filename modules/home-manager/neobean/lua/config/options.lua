...
if vim.g.neovim_mode == "skitty" then
  ...
  vim.cmd(string.format([[highlight WinBar1 guifg=%s]], colors["color03"]))
  -- -- Set the winbar to display "skitty-notes" with the specified color
  -- vim.opt.winbar = "%#WinBar1#   skitty-notes%*"
  -- Set the winbar to display the current file name on the left (without the extension) and Hackiri on the right
  vim.opt.winbar = '%#WinBar1# %{luaeval(\'vim.fn.fnamemodify(vim.fn.expand("%:t"), ":r")\')}%*%=%#WinBar1# Hackiri %*'
else
  ...
  vim.cmd(string.format([[highlight WinBar1 guifg=%s]], colors["color03"]))
  vim.cmd(string.format([[highlight WinBar2 guifg=%s]], colors["color02"]))
  vim.cmd(string.format([[highlight WinBar3 guifg=%s gui=bold]], colors["color24"]))
