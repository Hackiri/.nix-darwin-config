...
local cache = {
  ...
  branch_color = nil,
  commit_hash = "",
  file_permissions = { perms = "", color = colors["color03"] },
}
...
vim.api.nvim_create_autocmd({ "BufEnter", "FocusGained" }, {
  callback = function()
    ...
    cache.branch_color = (cache.branch == "live") and { fg = colors["color11"], gui = "bold" } or nil
...
vim.api.nvim_create_autocmd({ "BufWritePost", "BufEnter" }, {
  callback = function()
    if vim.bo.filetype ~= "sh" then
      cache.file_permissions = { perms = "", color = colors["color03"] }
      return
    end
    ...
    cache.file_permissions = {
      perms = permissions,
      color = (owner_permissions == "rwx") and colors["color02"] or colors["color03"],
    }
  end,
})
...
local function get_spell_bg_color()
  return vim.wo.spell and colors["color02"] or colors["color08"]
end
...
local function get_file_permissions()
  if vim.bo.filetype ~= "sh" then
    return "", colors["color03"]
  end
  local file_path = vim.fn.expand("%:p")
  local permissions = file_path and vim.fn.getfperm(file_path) or "No File"
  local owner_permissions = permissions:sub(1, 3)
  return permissions, (owner_permissions == "rwx") and colors["color02"] or colors["color03"]
end
...
--     end,
--     color = { fg = hostname_bg_color, bg = colors["color17"], gui = "bold" },
--     separator = { right = "" },
--     padding = 1,
--     cond = condition,
--   }
...
local function create_separator(condition)
  return {
    ...
    color = { fg = colors["color14"], bg = colors["color17"] },
  ...
  opts = function(_, opts)
    if vim.g.neovim_mode == "skitty" then
      ...
      opts.sections.lualine_b = {
        ...
        {
          get_venv_name,
          color = { fg = colors["color10"], bg = colors["color02"], gui = "bold" },
          separator = { right = "" },
        },
      }
      ...
      table.insert(opts.sections.lualine_x, 2, {
        ...
        color = function()
          return { fg = get_spell_bg_color(), bg = colors["color17"], gui = "bold" }
        end,
        separator = { left = "", right = "" },
        padding = 1,
      })
      ...
      table.insert(opts.sections.lualine_x, 4, {
        ...
        color = function()
          ...
          return { fg = bg_color, bg = colors["color17"], gui = "bold" }
        end,
        separator = { left = "", right = "" },
        padding = 1,
      })
